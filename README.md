...
- **`terraform apply` posture: validated + LocalStack-applied, never applied to real AWS.** Owner constraint is free tiers only; EKS control plane (~$73/mo) + NAT are never free. The stack passes `terraform fmt`/`validate` and `kubeconform -strict`, and the LocalStack-community-supported modules apply cleanly (`infrastructure/localstack/run.sh`, `docs/localstack-apply.md`); `docs/aws-free-tier.md` documents the $0 deployment path actually used. Remote state (S3 + DynamoDB locking, `infrastructure/backend.tf`) is declared per PRD §4.2 but the bucket/table are not bootstrapped — offline validation therefore uses `terraform init -backend=false`, and the LocalStack run overrides the backend to local state. The Route 53 `app.<domain>` ALIAS targets the CloudFront distribution but the distribution's alternate-domain-name + ACM cert (`cdn_aliases`/`cdn_acm_certificate_arn`) are left unset — a real apply against a real domain must wire those to the same host first (documented inline in `modules/route53/main.tf`); with the default mock domain the module `validate`s but is not apply-complete.
